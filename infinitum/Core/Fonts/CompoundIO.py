...
class _Dropdown:
    def __init__(self, width: int, height: int, buttons: List[Button],
                gap_color: Tuple[int, int, int], gap_size: int) -> None:
    ...
    def handle_event(self, event: pygame.event.Event, mouse_pos: Tuple[int, int], keys: pygame.key.ScancodeWrapper) -> None:
        for button, rect in self.buttons.items():
            if rect.collidepoint(*mouse_pos) and event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
                button.on_click()
...
class DropDownMenu:
    def __init__(self, pos: Tuple[int, int], width: int = 200, height: int = 30, dropup: bool = False, 
        ...
        self.menu = _Dropdown(width, height, buttons, gap_color, gap_size)
        self.window = Frame(process=self.menu, border=False, size=(width, self.menu.rect.y))
    ...
    def handle_event(self, event: pygame.event.Event, mouse_pos: Tuple[int, int]):
        self.window.handle_event(event, mouse_pos, active=True)
        self.visible = False